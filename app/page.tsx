...
const S: Record<string, React.CSSProperties> = {
  app: { maxWidth: 900, margin: '0 auto', padding: '1rem 0.75rem', fontFamily: 'inherit' },
  tabs: { display: 'flex', gap: 2, borderBottom: '1px solid #e0e0e0', marginBottom: '1.25rem', overflowX: 'auto' },
  card: { background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, padding: '1.25rem', marginBottom: '1rem' },
  formGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 },
  fg: { display: 'flex', flexDirection: 'column', gap: 4 },
  label: { fontSize: 12, color: '#888' },
  input: { fontSize: 14, padding: '8px 10px', border: '1px solid #d0d0d0', borderRadius: 8, background: '#fff', width: '100%', fontFamily: 'inherit' },
  btn: { display: 'inline-flex', alignItems: 'center', gap: 5, padding: '8px 16px', fontSize: 13, fontWeight: 500, border: '1px solid #d0d0d0', borderRadius: 8, cursor: 'pointer', background: '#fff' },
  btnPri: { background: '#111', color: '#fff', border: 'none' },
  btnDanger: { background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5' },
  secTitle: { fontSize: 15, fontWeight: 500, marginBottom: '1rem' },
  listHdr: { display: 'grid', gridTemplateColumns: '90px 70px 1fr 90px 56px 120px', gap: 6, padding: '6px 0', fontSize: 11, fontWeight: 500, color: '#999', borderBottom: '1px solid #f0f0f0' },
  listRow: { display: 'grid', gridTemplateColumns: '90px 70px 1fr 90px 56px 120px', gap: 6, padding: '9px 0', borderTop: '1px solid #f5f5f5', fontSize: 12, alignItems: 'center' },
  lockedRow: { background: '#fafafa', borderRadius: 8, padding: '9px 8px', margin: '2px 0' },
  tag: { fontSize: 10, padding: '2px 6px', borderRadius: 6, background: '#f0f0f0', color: '#666', whiteSpace: 'nowrap' as const },
  editBtn: { fontSize: 11, padding: '3px 7px', border: '1px solid #e0e0e0', borderRadius: 4, cursor: 'pointer', background: 'transparent', color: '#666', whiteSpace: 'nowrap' as const },
  chip: { display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 10, padding: '2px 7px', borderRadius: 4, background: '#f5f5f5', color: '#999', border: '1px solid #e8e8e8' },
  modalBg: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' },
  modal: { background: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, padding: '1.25rem', width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' as const },
  notice: { background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#92400e', lineHeight: 1.7 },
  adminBar: { display: 'flex', alignItems: 'center', gap: 8, background: '#ecfdf5', border: '1px solid #6ee7b7', borderRadius: 8, padding: '8px 12px', marginBottom: '1rem', fontSize: 12, color: '#065f46' },
  monthHdr: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f0f0f0', marginBottom: 4 },
  calHdr: { display: 'grid', gridTemplateColumns: '72px repeat(4, 1fr)', fontSize: 11, fontWeight: 500, color: '#999' },
  calRow: { display: 'grid', gridTemplateColumns: '72px repeat(4, 1fr)', borderTop: '1px solid #f5f5f5', alignItems: 'stretch' },
  calSlot: { padding: '5px 3px', borderLeft: '1px solid #f5f5f5', minHeight: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' },
}

type Tab = 'apply' | 'cal' | 'list'
type ModalMode = 'detail' | 'edit' | 'delete' | 'notice' | 'adminLogin' | null
...
export default function Home() {
  ...
  useEffect(() => {
    if (tab === 'list' || tab === 'cal') fetchRecords()
  }, [tab, fetchRecords])

  // 탭 전환
  const switchTab = (t: Tab) => {
    setTab(t)
    if (t === 'list') setLoadedMonths(1)
  }
  ...
  const openEdit = (r: Dispatch) => {
    setSelectedId(r.id)
    setEDate(r.date); setECar(r.car); setEDept(r.dept)
    setEUser(r.user); setEPassengers(r.passengers || '')
    setEDesc(r.description || ''); setEDest(r.dest)
    setEDepart(r.depart || ''); setEArrive(r.arrive || '')
    setEFuel(r.fuel || 0); setEKm(r.km || '')
    setModal('edit')
  }
  ...
  const tryAdminLogin = () => {
    if (adminPwInput === ADMIN_PW) {
      setIsAdmin(true); setModal(null); setPwError(false); setAdminPwInput('')
    } else {
      setPwError(true); setAdminPwInput('')
    }
  }
  ...
  const hasMore = records.some(r => {
    const [ry, rm] = r.date.split('-').map(Number)
    return ry * 100 + (rm - 1) < oldestKey
  })
  ...
  return (
    <div style={S.app}>
      ...
      {tab === 'apply' && (
        <div style={S.card}>
          <p style={S.secTitle}>배차 신청서</p>
          <div style={S.formGrid}>
            <div style={S.fg}>
              <label style={S.label}>사용일자</label>
              <input style={S.input} type="date" value={fDate} onChange={e => setFDate(e.target.value)} />
            </div>
            <div style={S.fg}>
              <label style={S.label}>차량 종류</label>
            ...
            <div style={S.fg}>
              <label style={S.label}>사용자 (운전자)</label>
              <input style={S.input} type="text" placeholder="예) 홍길동" value={fUser} onChange={e => setFUser(e.target.value)} />
            </div>
            <div style={{ ...S.fg, gridColumn: '1 / -1' }}>
              <label style={S.label}>동승인</label>
              <input style={S.input} type="text" placeholder="예) 김철수, 이영희 (없으면 비워두세요)" value={fPassengers} onChange={e => setFPassengers(e.target.value)} />
            </div>
            <div style={{ ...S.fg, gridColumn: '1 / -1' }}>
              <label style={S.label}>운행내용</label>
              <input style={S.input} type="text" placeholder="예) 시청 업무 협의" value={fDesc} onChange={e => setFDesc(e.target.value)} />
            </div>
            <div style={{ ...S.fg, gridColumn: '1 / -1' }}>
              <label style={S.label}>목적지</label>
              <input style={S.input} type="text" placeholder="예) 시청, 우체국" value={fDest} onChange={e => setFDest(e.target.value)} />
            </div>
            <div style={S.fg}>
      ...
      {tab === 'cal' && (
        <div style={S.card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1rem' }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 500 }}>{calY}년 {calM + 1}월</span>
            <button style={{ background: 'none', border: '1px solid #e0e0e0', borderRadius: 8, padding: '4px 10px', cursor: 'pointer', fontSize: 16 }} onClick={() => setCalMonth(new Date(calY, calM + 1, 1))}>›</button>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <div style={{ minWidth: 420 }}>
              <div style={S.calHdr}>
                <div style={{ padding: '4px 6px', textAlign: 'center' }}>날짜</div>
                {CARS.map(car => (
                  <div key={car} style={{ padding: '4px 6px', textAlign: 'center' }}>
                    {car.split(' ')[0]}<br />
              ...
              {Array.from({ length: calDays }, (_, i) => i + 1).map(d => {
                ...
                return (
                  <div key={d} style={S.calRow}>
                    <div style={{ fontSize: 11, color: isWeekend ? '#ef4444' : '#999', padding: '7px 6px', display: 'flex', alignItems: 'center' }}>
                      {calM + 1}/{d}<br /><span style={{ fontSize: 10 }}>({DOWS[dow]})</span>
                    </div>
                    {CARS.map(car => {
                      const rec = records.find(r => r.date === ds && r.car === car)
                      return (
                        <div key={car} style={S.calSlot}>
                          {rec ? (
                            <span
                              onClick={() => { setSelectedId(rec.id); setModal('detail') }}
                              style={{ fontSize: 10, padding: '2px 5px', borderRadius: 3, cursor: 'pointer', background: rec.locked ? '#f5f5f5' : '#ecfdf5', color: rec.locked ? '#999' : '#065f46' }}
                            >
                              {rec.locked ? '🔒 ' : ''}{rec.dept}
                            </span>
                          ) : (
        ...
        <>
          ...
          <div style={S.card}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: 8 }}>
              <p style={{ ...S.secTitle, margin: 0 }}>신청 내역</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 12, color: '#bbb' }}>
                  {loadedMonths === 1
                    ? `${now.getFullYear()}년 ${now.getMonth() + 1}월`
                    : `${oldest.y}년 ${oldest.m + 1}월 ~ ${now.getFullYear()}년 ${now.getMonth() + 1}월`}
                </span>
            ...
            {monthKeys.map(({ y, m }) => {
              ...
              return (
                <div key={key} style={{ marginBottom: '1.25rem' }}>
                  <div style={S.monthHdr}>
                    <span style={{ fontSize: 13, fontWeight: 500 }}>
                      {y}년 {m + 1}월
                      {isNow && <span style={{ fontSize: 11, color: '#1D9E75', fontWeight: 400, marginLeft: 6 }}>이번 달</span>}
                    </span>
                    <span style={{ fontSize: 11, color: '#bbb' }}>{monthRecs.length}건</span>
                  </div>
                  <div style={S.listHdr}>
                    <span>사용일자</span><span>차량</span><span>사용자 / 운행내용</span>
                    <span>목적지</span><span>유류</span><span>관리</span>
                  </div>
                  {monthRecs.map(r => (
                    <div key={r.id} style={{ ...S.listRow, ...(r.locked ? S.lockedRow : {}) }}>
                      <span>{r.date}</span>
                      <span style={S.tag}>{r.car.split(' ')[0]}</span>
                      <div>
                        <span style={{ fontWeight: 500 }}>{r.user}{r.passengers ? ` (${r.passengers})` : ''}</span><br />
                      ...
                      <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', alignItems: 'center' }}>
                        {r.locked && <span style={S.chip}>🔒 확인완료</span>}
            ...
            {!loading && records.filter(r => {
              const [ry, rm] = r.date.split('-').map(Number)
              return monthKeys.some(({ y, m }) => ry === y && rm - 1 === m)
            }).length === 0 && (
            ...
            {hasMore && (
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: '0.5rem' }}>
                <button style={{ ...S.btn, width: '100%', justifyContent: 'center', color: '#999' }} onClick={() => setLoadedMonths(v => v + 1)}>
                  ▲ 이전 달 내역 불러오기
                </button>
              </div>
            )}
      ...
      {modal === 'notice' && (
        <div style={S.modalBg}>
          <div style={{ ...S.modal, maxWidth: 360, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>운행 후 꼭 확인해 주세요</p>
      ...
      {modal === 'detail' && selectedRecord && (
        <div style={S.modalBg} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={S.modal}>
            <p style={{ fontSize: 15, fontWeight: 500, marginBottom: 4 }}>배차 상세 정보</p>
            ...
            {[
              ...
              <div key={l} style={{ display: 'flex', gap: 8, padding: '7px 0', borderTop: '1px solid #f5f5f5' }}>
                <span style={{ fontSize: 12, color: '#999', minWidth: 110 }}>{l}</span>
      ...
      {modal === 'edit' && (
        <div style={S.modalBg} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={S.modal}>
            <p style={{ fontSize: 15, fontWeight: 500, marginBottom: 4 }}>신청 내역 수정</p>
            <p style={{ fontSize: 12, color: '#999', marginBottom: '1rem' }}>{eDate} · {eCar} · {eDept}</p>
            <div style={{ ...S.formGrid, gridTemplateColumns: '1fr 1fr' }}>
              ...
              <div style={S.fg}><label style={S.label}>차량 종류</label>
                <select style={S.input} value={eCar} onChange={e => setECar(e.target.value)}>
                  {CARS.map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div style={S.fg}><label style={S.label}>부서</label>
                <select style={S.input} value={eDept} onChange={e => setEDept(e.target.value)}>
                  {DEPTS.map(d => <option key={d}>{d}</option>)}
                </select>
              </div>
              ...
              <div style={{ ...S.fg, gridColumn: '1/-1' }}>
                <label style={S.label}>유류(배터리)잔량</label>
                <FuelBar value={eFuel} onChange={setEFuel} />
              </div>
      ...
      {modal === 'delete' && selectedRecord && (
        <div style={S.modalBg} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={S.modal}>
            <p style={{ fontSize: 15, fontWeight: 500, marginBottom: 4 }}>내역 삭제</p>
      ...
      {modal === 'adminLogin' && (
        <div style={{ ...S.modalBg, zIndex: 200 }} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={{ ...S.modal, maxWidth: 320, textAlign: 'center' }}>
            <p style={{ fontSize: 15, fontWeight: 500, marginBottom: 6 }}>관리자 로그인</p>
